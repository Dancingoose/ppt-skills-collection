...
1. Do not create `preview.html`, `design.html`, or a PPTX until this intake is complete. Do not create design directions until the first three batches are complete and the pre-design intent gate passes. Material may inform recommendations and answer choices, but never replace a creator response.
...
5. After batch 3, write the first 12 answers and three batch records to `decision.intentQuestionnaire` with `schemaVersion: 3`, `skill: "ppt-workflow-intake"`, and `completed: false`, then pass the pre-design intent gate. After the visual samples are shown and batch 4 is answered, replace it with exactly one response for every required ID, `completed: true`, and one non-empty `creatorConfirmation` for each batch in order. Every answer must have `source: "creator-confirmed"` and evidence pointing to the corresponding creator reply.
