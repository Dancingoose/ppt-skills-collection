...
1. Run `check_workflow_state.py` for intent, preparation, decision, and execution. Resolve every failure, including creator-confirmed intake, review, effects-scan artifacts, and any declared presentation-protocol drift, before conversion. The protocol is verified by the execution gate and does not replace source, review, or audit evidence.
...
5. Record the final output filename, audit result, reviewed page count, SHA-256 of the audited PPTX, and specific findings in `delivery.audit`. For V3, also record `unresolvedHighRiskPages: []` and `structuralWarnings: 0`; the execution gate must already contain a passing `intent-continuity-review.json` that covers the approved preview and current HTML. The file-backed audit must contain matching `checks.preflight` and `checks.structuralSelfCheck` evidence with passing statuses and zero findings. Delivery is incomplete until the delivery gate passes; any PPTX change after review invalidates the audit.
