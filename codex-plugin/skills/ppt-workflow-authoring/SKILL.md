...
1. Run `check_workflow_state.py --layer decision` and resolve every failure before authoring a preview or full deck. Use only the selected `decision.designRecipes` entry and confirm `decision.designOrchestration` still binds every bridge artifact, sample, and selected recipe. Read `references/quick-reference-card.md`, then only the selected theme block from `references/theme-tokens.md`, the chosen layouts in `references/layout-library.md`, and the native-size rule for the selected canvas in `references/canvas-formats.md`.
...
3. Read `references/web-image-sourcing.md` before placing a non-data image. Build `design.html` at the native canvas. Each `.slide` must carry `data-pptx-slide`, `data-slide-id`, `data-layout`, `data-item-count`, `data-color-system`, and the selected `data-design-recipe` matching `workflow-state.json` and the post-build color review. The explicit marker is required because a small or ultra-wide native canvas can fall below the converter's geometric discovery threshold. Keep facts traceable to the inventory and preserve supplied images when they are evidence rather than decoration. For missing imagery, use only a licence-verified image recorded in `image-sourcing-plan.md`, or the page's recorded no-image layout; never generate, request, or use AI-generated imagery. ECharts data charts are outside this image rule.
...
5. After the final `design.html` and execution manifest are complete, run `python <collection-root>/ppt-workflow/scripts/presentation_protocol.py sync --task <task-dir>`. This writes the task-local protocol sidecar and binds it to `execution.protocol`; rerun it after any HTML or manifest change, before the execution gate.
6. Use the HTML-first converter only. Do not author the deliverable shape-by-shape with `python-pptx` or `pptxgenjs`.
