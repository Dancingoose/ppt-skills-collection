...
7. Run the validator after preparation, design decision, execution, and delivery. Resolve every failure. On a new machine, run `<collection-root>/html-to-pptx/scripts/bootstrap-runtime.ps1` once. Run `python <collection-root>/html-to-pptx/convert.py` only after the delivery gate confirms its runtime can render a page.
