...
WORKFLOW = ROOT / "ppt-workflow" / "SKILL.md"
...
class PluginDesignOrchestratorTests(unittest.TestCase):
    ...
    def test_studio_makes_missing_design_evidence_a_hard_stop(self):
        ...
        self.assertIn("--layer design", text)

    def test_canonical_workflow_uses_fixed_chain_without_a_fast_path(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn(
            "`claude-design -> ui-ux-pro-max -> mbb-decks -> frontend-design -> axi-front-design`",
            text,
        )
        self.assertIn("不得按场景跳过", text)
        self.assertIn("--layer design", text)
