...
def first_font(font_family: str) -> str:
    ...
    # 走到这里说明整条 stack 全是 generic 关键字（如 font-family: system-ui）——
    # 不能把关键字本身写进 typeface（PowerPoint/WPS 找不到，faux-bold 双绘），用兜底
...
def add_shape_box(slide, rec):
    ...
    if deco.get("hasBg"):
        ...
        if a_ < 1.0:
            _set_fill_alpha(shape, a_)
        if is_round and len(active_sides) == 4 and same_border_colors:
            # 圆 / pill 的边框无法按边拆成直线；四边同色时直接设为 shape outline，
...
def _annotate_baseline_offsets(runs):
    """把 CSS 行内上下标（<sup>/<sub> 的 vertical-align）翻译成 OOXML baseline 偏移。

    参考基线 = 同段 verticalAlign=baseline 的最大字号 run 的墨迹底边；
    字体 descent 用 0.2em 近似。偏移显著（≥5% 自身字号）才标注，
    超过参考字号 1.2 倍的跨行假偏移丢弃。结果写进 run["baselinePct"]（1/1000 %）。
    """
    ref = None
    for run in runs:
        if run.get("linebreak") or run.get("inkBottom") is None:
            continue
        fs = float(run.get("fontSize") or 16)
        if str(run.get("verticalAlign", "baseline")) == "baseline" \
                and (ref is None or fs > ref[0]):
            ref = (fs, float(run["inkBottom"]))
    if ref is None:
        return
    ref_fs, ref_ib = ref
    for run in runs:
        ...
        if str(run.get("verticalAlign", "baseline")) == "baseline":
            continue
        fs = float(run.get("fontSize") or 16)
        offset_px = (ref_ib - 0.2 * ref_fs) - (float(run["inkBottom"]) - 0.2 * fs)
        if abs(offset_px) > ref_fs * 1.2:
            continue  # 与参考 run 不在同一视觉行，偏移无意义
        pct = int(round(offset_px / fs * 100000))
        if abs(pct) >= 5000:
...
def _emit_run(paragraph, text, run, text_transform):
    ...
    if exact_weight_face:
        ...
        bold = False
        italic = False
    cr, cg, cb, color_alpha = parse_rgba(run.get("color", "rgb(0,0,0)"))
    color_rgb = (cr, cg, cb)
    ...
    if run.get("baselinePct"):
        rPr.set("baseline", str(run["baselinePct"]))
    rPr.set("dirty", "0")

    # fill color（rgba 文字色的 alpha 必须写 <a:alpha>，否则暗底上的
    # 半透明弱化文字（kicker / 页脚 / muted label）会变成不透明主色）
    solidFill = etree.SubElement(rPr, qn("a:solidFill"))
    srgb = etree.SubElement(solidFill, qn("a:srgbClr"))
    srgb.set("val", "{:02X}{:02X}{:02X}".format(*color_rgb))
    if color_alpha < 1.0:
        alpha_el = etree.SubElement(srgb, qn("a:alpha"))
        alpha_el.set("val", str(int(round(color_alpha * 100000))))
    ...
    elif any(ord(ch) > 0x7F for ch in text):
        # PowerPoint 把 × ÷ ° ± 等 "EA-ambiguous" 码点路由到 ea 槽；纯 Latin run
        # 不写 ea 时它们落到主题默认 EA 字体（与相邻粗体数字不匹配的细字形）。
        # ea 指回 latin 同字体即可；同字体同 metrics，不触发 WPS 的
        # "ea advance width 应用到整 run" 问题（那条 lesson 针对的是 CJK 字体作 ea）
        ea_el = etree.SubElement(rPr, qn("a:ea"))
    ...
    for rec in data["records"]:
        if rec["kind"] == "shape":
            ...
            if rec["rect"]["w"] >= SLIDE_W_PX * 0.99 and rec["rect"]["h"] >= SLIDE_H_PX * 0.99:
                ...
                borderless = not any(deco.get(k) for k in
                                     ("borderTop", "borderBottom", "borderLeft", "borderRight"))
                same_as_bg = True
                if deco.get("hasBg"):
                    r_, g_, b_, a_ = parse_rgba(deco.get("bg") or "")
