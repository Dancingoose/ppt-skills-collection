...
- Canvas/WebGL→PPTX降级为静态截图（deco_snapshot档，1920×1080 PNG）
- ECharts 走截图保留视觉；着色器等2-3帧初始化后再截

## 执行门禁速查（6 个 Gate，不可跳过）
| Gate | 位置 | 检查 | 不过的处理 |
|------|------|------|-----------|
| G1 Phase1 前置 | 决策层入口 | AskUserQuestion 完成（受众/意图/主张/画布） | 先问用户再加载 claude-design |
...
- **决策层**：`check --layer decision` → Phase1完成 | 按场景选skill | 方向名精确 | Phase2完成 | 反模板审查 | 护照4字段填
